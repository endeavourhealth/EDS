...
public abstract class EmisCsvTransformer {
    ...
    public static List<UUID> splitAndTransform(String[] files,
        ...
        EmisCsvTransformerWorker.transform(dstDir, processor);
        ...
        while (!pool.awaitTermination(1, TimeUnit.HOURS)) {
            ...
            int countOutstanind = 0;
            for (Future future: futures) {
                if (!future.isDone()) {
                    countOutstanind ++;
                } else {

                    //if the future is completed, calling get() on it will cause it to raise
                    //any exception that was generated by that callable, which we then just throw again
                    try {
                        future.get();
                    } catch (Exception ex) {
                        throw ex;
            ...
            LOG.trace("Waiting for {} transforms to complete", countOutstanind);
        }
