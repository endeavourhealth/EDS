...
public interface LibraryAccessor {
    ...
    @Query("SELECT * FROM admin.active_item_by_org WHERE organisation_id = :organisation_id and is_deleted = false limit 10")
    ...
    @Query("SELECT * FROM admin.audit_by_org_and_date_desc WHERE organisation_id = :organisation_id limit 10")
