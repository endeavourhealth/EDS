...
) COMMENT 'To track PCR data back to source if needed. For problems and audit';
...
) COMMENT 'Sources of data - url of rdbms server ';
...
(
  ...
  entry_practitioner_id int COMMENT 'who actually recorded the entry into the host system',
  device_id             int COMMENT 'refers to the device that may have recorded the data',
  entry_mode            tinyint  NOT NULL COMMENT 'entry mode i.e. 1-upsert,0-delete',
  table_name            tinyint  NOT NULL COMMENT 'lookup to pcr_tables',
  ...
  CONSTRAINT event_log_entered_practitioner_id
    FOREIGN KEY (entry_practitioner_id)
      REFERENCES practitioner (id)
