...
create table procedure_procedure (
    ...
    freetext_comment varchar(255) COMMENT 'from Comment',
...
create table procedure_PROCE (
    ...
    encounter_id int NOT NULL COMMENT 'from ENCNTR_ID',
    procedure_dt_tm datetime COMMENT 'from PROCEDURE_DT_TM',
    procedure_type varchar(50) NOT NULL COMMENT 'opcs or snomed, derived from CONCEPT_CKI_IDENT',
    procedure_code varchar(50) NOT NULL COMMENT 'opcs-4 or snomed code, opcs-4 code in format ANN.N, derived from CONCEPT_CKI_IDENT',
    procedure_term varchar(255) NOT NULL COMMENT 'corresponding term for the above code, looked up via TRUD',
    procedure_seq_nbr int COMMENT 'from PROCEDURE_SEQ_NBR',
    lookup_person_id int COMMENT 'pre-looked up via ENCNTR_ID',
    lookup_mrn varchar(10) NOT NULL COMMENT 'looked up via ENCNTR_ID',
    lookup_nhs_number varchar(10) NOT NULL COMMENT 'patient NHS number, looked up via ENCNTR_ID',
    lookup_date_of_birth datetime NOT NULL COMMENT 'patient NHS number, looked up via ENCNTR_ID',
	audit_json mediumtext null comment 'Used for Audit Purposes',
...
create table procedure_SURCP (
    ...
    dt_extract datetime NOT NULL COMMENT 'from EXTRACT_DT_TM',
    active_ind bool NOT NULL COMMENT 'whether an active record or not (deleted), from ACTIVE_IND',
    procedure_code int NOT NULL COMMENT 'from PROC_CD, relates to CVREF - not an OPCS or Snomed code',
    procedure_text varchar(255) NOT NULL COMMENT 'from PROC_TXT',
    modifier_text varchar(255) NOT NULL COMMENT 'from MODIFIER_TXT',
